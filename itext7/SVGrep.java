...
public class SVGrep {
     ...
     */
    public static Exception addSVG(SVGrep SVGdoc, PdfContentByte cb, float x, float y) {
        if (SVGdoc.success) {
            float corr = 0.75f; // correction factor to cause the SVG to appear the same size in PDF as in HTML
            try {
                if (SVGdoc.dims != null) {
                    Graphics2D g2d;
                    PdfTemplate map = cb.createTemplate(SVGdoc.dims[0] * corr, SVGdoc.dims[1] * corr);
                    //g2d = map.createGraphics(SVGdoc.dims[0] * corr, SVGdoc.dims[1] * corr, new DefaultFontMapper());
                    g2d = new PdfGraphics2D(map, SVGdoc.dims[0] * corr, SVGdoc.dims[1] * corr, new DefaultFontMapper());
                    g2d.scale(corr, corr);
    ...
    static Float[] getSvgDims(javax.xml.transform.stream.StreamSource svg) {
        final String XSLT = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
                ...
                + "</xsl:stylesheet>";
        try {
            il.co.aman.apps.XsltTransform transform = new il.co.aman.apps.XsltTransform(new javax.xml.transform.stream.StreamSource(new java.io.ByteArrayInputStream(XSLT.getBytes("UTF-8"))));
